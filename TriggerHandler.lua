...
function TriggerHandler:isAllowedToLoadSeparateFillType(object,dataLength,fillType)
	...
	if dataLength>1 and #self.lastLoadedFillTypes < separateFillTypeLoading:get() then 
		for f,_ in pairs(self.lastLoadedFillTypes) do 
			if f == fillType then
...
function TriggerHandler.handleLoadTriggerCallback(self,triggerId, otherId, onEnter, onLeave, onStay, otherShapeId,rootVehicle,fillableObject)
	...
	if onLeave then 
		...
		for id,object in pairs(self.fillableObjects) do 
			if object.getRootVehicle then 
				stillInTrailer = object:getRootVehicle() == rootVehicle
			end
		end
